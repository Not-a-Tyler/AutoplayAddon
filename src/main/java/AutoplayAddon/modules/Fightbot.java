...
public class Fightbot extends Module {
                ...
                if (targetName.equals(targetPlayer.getGameProfile().getName())) {
                    ...
                    if (mc.player.getAttackCooldownProgress(0.5f) >= 1) {
                        ...
                        new Thread(() -> {
                            ...
                            GotoUtil.setPos(targetPlayer.getPos(), false, true, false);
                            Packet.sendPacket(PlayerInteractEntityC2SPacket.attack(targetPlayer, false));
                        }).start();
    ...
    private void handlePlayerMovement(PlayerEntity targetPlayer) {
        ...
        if (desiredPos != null) {
            new Thread(() -> {
                GotoUtil.setPos(desiredPos, false, true, true);
            }).start();
        }
    }
